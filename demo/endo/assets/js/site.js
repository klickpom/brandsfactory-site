...
(function () {
  ...
  function fillStatic() {
    ...
    if (mapKicker) mapKicker.textContent = '٣ فروع · القاهرة';
  ...
  function renderTicket() {
    ...
    document.getElementById('ticket-branch').textContent = br ? br.name : 'اختار فرع';
  ...
  function showSuccess(b) {
    ...
    document.getElementById('bs-summary').innerHTML = `
      ...
      <div><dt>الفرع</dt><dd>${b.branchName || ''}</dd></div>
  ...
  function selectMapBranch(id) {
    ...
    if (kicker) kicker.textContent = 'فرع ' + br.name;
