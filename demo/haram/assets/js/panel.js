...
(function () {
  ...
  function renderDashboard() {
    ...
    root().innerHTML = `
      <div class="summary-strip" aria-label="ملخص العيادة">
        ...
        <button type="button" class="summary-item" data-jump="all"><b>${D.branches.length}</b><span>دكاترة زائرين في نظام واحد</span></button>
      </div>
  ...
  function renderBookings() {
    ...
    host.innerHTML = `
      ...
      <div class="filter-pills" role="group" aria-label="فلترة حسب الدكتور">
        <button class="filter-pill" data-branch="all" aria-pressed="${activeBranch === 'all'}">الدكتورين</button>
        ${D.branches.map(br => {
      ...
      <div class="search-box">
        ...
        <input type="search" id="appt-search" placeholder="دوّر بالاسم أو الموبايل أو رقم الحجز أو الدكتور…" aria-label="بحث في الحجوزات">
      </div>
    ...
    host.querySelectorAll('[data-branch]').forEach(btn =>
      btn.addEventListener('click', () => {
  ...
  function filteredBookings() {
    ...
    arr = sortInbox(arr);
    if (activeBranch !== 'all') {
      arr = arr.filter(b => b.branchId === activeBranch);
    }
    const q = apptQuery.trim();
    if (q) {
      arr = arr.filter(b =>
        b.name.includes(q) || b.phone.includes(q) || (b.ref || '').includes(q) ||
        b.service.includes(q) || (b.branchName || '').includes(q));
    }
    return arr;
  }
  ...
  function renderTemplates() {
    ...
    host.innerHTML = `
      ...
      <p class="section-sub">أي تعديل هنا بيغيّر الرسالة اللي بتتبعت فوراً. المتغيرات المتاحة: {الاسم} {الرقم} {الخدمة} {التخصص} {اليوم} {الوقت} {الموبايل}</p>
