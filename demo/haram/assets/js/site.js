...
(function () {
  ...
  function workingDays() {
    return BF.nextWorkingDays(D.bookingWindowDays, book.branchId);
  }
  ...
  function fillStatic() {
    ...
    if (phoneLink) {
      ...
      const phoneWrap = phoneLink.closest('p');
      if (phoneWrap) phoneWrap.hidden = true;
    }

    document.getElementById('footer-wa').href =
      BF.waLink(D.clinic.phoneIntl, 'أهلاً، عايز أحجز موعد من واتساب');
    ...
    document.getElementById('map-wa').href =
      BF.waLink(D.clinic.phoneIntl, 'أهلاً، عايز أحجز موعد في عيادة الهرم');
    const mapKicker = document.getElementById('map-kicker');
    const mapTitle = document.getElementById('map-title');
    if (mapKicker) mapKicker.textContent = 'الهرم · الجيزة';
    if (mapTitle) mapTitle.textContent = D.clinic.name;
    ...
    if (places) {
      places.innerHTML = D.branches.map(br => `
        <button type="button" class="branch-place" data-map-branch="${br.id}">
          <b>${br.name}</b>
          <span>${br.daysLabel || br.address}</span>
        </button>`).join('');
    ...
    const visit = document.getElementById('visit-grid');
    if (visit) {
      visit.innerHTML = D.branches.map(br => `
        <article class="visit-card">
          <span class="doc-ring sm"><span class="doc-initials">${br.initials || 'د'}</span></span>
          <div>
            <h3>${br.name}</h3>
            <p>${br.doctor} — ${br.daysLabel}</p>
            <button type="button" class="btn btn-ghost btn-sm" data-visit-branch="${br.id}">احجز للدكتور ده</button>
          </div>
        </article>`).join('');
      visit.querySelectorAll('[data-visit-branch]').forEach(btn => {
        btn.addEventListener('click', () => {
          pickBranch(btn.dataset.visitBranch);
          setStep(1);
  ...
  function renderTicket() {
    ...
    document.getElementById('ticket-branch').textContent = br ? br.name : 'اختار الدكتور';
  ...
  function renderBranches() {
    ...
    host.innerHTML = D.branches.map(br => {
      ...
      return `<button type="button" class="branch-chip" role="radio"
                ...
                <b>${br.name}</b>
                <small>${br.daysLabel || br.address}</small>
              </button>`;
    }).join('');
  }
  ...
  function renderLiveSlots() {
    ...
    host.innerHTML = free.length
      ...
      : '<p class="slot-hint" style="padding:14px">كل المواعيد محجوزة الفترة الجاية. ابعتلنا واتساب</p>';
  ...
  function showSuccess(b) {
    ...
    document.getElementById('bs-summary').innerHTML = `
      ...
      <div><dt>الدكتور</dt><dd>${b.branchName || ''}</dd></div>
  ...
  function selectMapBranch(id) {
    ...
    if (kicker) kicker.textContent = 'الهرم · الجيزة';
    if (title) title.textContent = br.name;
    document.getElementById('map-address').textContent = br.daysLabel || D.clinic.address;
    document.getElementById('map-directions').href = D.clinic.mapsUrl;
